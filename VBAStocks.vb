Sub vbaChallenge()
  ...
  Range("n3").Value = "Greatest % Descrease"
...
'--------------Loop throu all stocks and get total volume for each, begin populating summary section ---------------------- '
  ...
  For I = 2 To lastrow
    ...
    If Cells(I + 1, 1).Value <> Cells(I, 1).Value Then
      ...
      Range("l" & rowNum).Value = totalStock
...
' Print the yearly change to the summary section
   ...
   'dim % Change
    ...
    Range("k" & rowNum).Value = percentChange
...
'---------Loop through summary section to get greatest % increase, % decrease, and total volume-------------------
  ...
  'find max change in % change column
    If Cells(w, 11).Value = WorksheetFunction.Max(Range("k2:k" & lastrow2)) Then
        ticker2 = Cells(w, 9).Value
  'set greatest % increase
        greatestInc = Cells(w, 11).Value
   'print stock and gpi
        Range("o2") = ticker2
        Range("p2") = greatestInc
        
  'find min change in % change column
    ElseIf Cells(w, 11).Value = WorksheetFunction.Min(Range("k2:k" & lastrow2)) Then
 'set greatest % increase
      ticker2 = Cells(w, 9).Value
      greatestDec = Cells(w, 11).Value
   'print stock and gpd
        Range("o3") = ticker2
        Range("p3") = greatestDec

  'find max volume
    ElseIf Cells(w, 12).Value = WorksheetFunction.Max(Range("l2:l" & lastrow2)) Then
   'set greatest total volume
      ticker2 = Cells(w, 9).Value
      greatestVol = Cells(w, 12).Value
   'print stock and gtv
        Range("o4") = ticker2
        Range("p4") = greatestVol
